"""Corrected I+II+IV runner with candidate CIK linkage. Default: smoke only.
...
def attach_text(frame, values, text_path, tech_hash):
    ...
    if tr['status']!='DAILY_TEXT_BUILT_CANDIDATE_LINKAGE_REQUIRES_REVIEW':
        raise ValueError('Unrecognized text build protocol; review before mixing runs')
    ...
    audit={'text_columns':TEXT,'auxiliary_columns':adapter.FUND+TEXT,
           ...
           'text_linkage_status':'CANDIDATE_NOT_HISTORICALLY_VERIFIED',
...
def main():
    ...
    p.add_argument('--text',default='datasets/daily_text_20260920T220718Z_8b07a12d/text_daily.csv')
    ...
    out=root/'results'/(('corrected_I_II_IV_'+a.mode+'_candidate_seed')+str(a.seed)+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid4().hex[:8])
    out.mkdir(parents=True,exist_ok=False)
    start=time.perf_counter();rp=out/'experiment_report.json'
    report=dict(status='RUNNING',protocol='corrected_prices_permno_I_II_IV_candidate_v1',mode=a.mode,final_thesis_ready=False,seed=a.seed,output_folder=str(out),
