"""Purged full-panel I+II experiment: selectable seed (default 42), at most 30 epochs.
...
def main():
    ...
    seed_all()
    out = root / 'results' / (f'purged_I_II_seed{args.seed}_' + datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ') + '_' + uuid4().hex[:8])
    out.mkdir(parents=True, exist_ok=False)
    print(f'FULL EXPERIMENT | I+II | seed {args.seed} | up to 30 epochs | batch size 64', flush=True)
    ...
    report = dict(status='running', protocol='purged_fixed_alpha1_v1', purpose=f'purged_I_II_seed{args.seed}_full_training',
    ...
    try:
        ...
        with isolated_outputs(out):
            result = train_pytorch_model(
                ...
                model_name=f'PURGED_I_II_seed{args.seed}', epochs=30, device=device,
        ...
        report_path.write_text(json.dumps(report, indent=2), encoding='utf-8')
    print(f"PASS | purged I+II seed{args.seed} | training + evaluation + export: {report['training_and_validation_minutes']:.1f} min", flush=True)
