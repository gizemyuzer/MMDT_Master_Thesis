"""Full corrected technical-only (I) experiment. Keep run_corrected_smoke.py beside this file.
...
def main():
    ...
    out=root/'results'/('corrected_I_seed'+str(a.seed)+'_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')+'_'+uuid4().hex[:8])
    out.mkdir(parents=True,exist_ok=False)
    start=time.perf_counter();rp=out/'experiment_report.json'
    report=dict(status='RUNNING',protocol='corrected_prices_permno_I_v1',seed=a.seed,output_folder=str(out),
                ...
                limitations=['Compustat RDQ alignment is not vintage point-in-time financial data.',
                             'Technical-only I. Fundamental sidecar is loaded for identical cohort/preprocessing checks but never used by the model. One seed does not establish a modality effect.'])
    def save():
        report['elapsed_minutes']=(time.perf_counter()-start)/60
        rp.write_text(json.dumps(report,indent=2),encoding='utf-8')
    save();print('FULL CORRECTED I | seed',a.seed,'| max 30 epochs',flush=True)
    ...
    try:
        ...
        report.update(audit)
        expected_sources = {
            'technical': 'b7a9de11255319edebcb0c1042ca4cd60da2c66ab3fe906beaca3549eea45287',
            'fundamental': 'f7763fde9f81758ccf3c3b0df3265d5b6a782a809defb1d135ffe2ce55ebe731',
        }
        if audit['source_sha256'] != expected_sources:
            raise ValueError('Source files differ from completed I+II protocol; do not mix these runs.')
        report['loaded_fundamental_columns'] = list(adapter.FUND)
        report['fundamental_columns'] = []
        report['active_input_columns'] = list(adapter.TECH)
        report['ablation'] = 'tech_only: no fundamental encoder or fusion; same cohort and training rules as I+II'
        report['sequence_counts']['TestEndpoint'] = len(pos['TestEndpoint'])
        ...
        config=dict(tech_dim=32,fund_dim=6,seq_len=20,d_model=64,n_heads=4,n_layers=2,ffn_dim=256,
                    dropout=.15,modality='tech_only',fusion_type='cross_attention')
        ...
        if hasattr(model, 'fund_encoder') or hasattr(model, 'fusion_mlp'):
            raise AssertionError('Technical-only model unexpectedly contains a fundamental/fusion module')
        # Prove that the unused stream cannot affect predictions, without consuming
        # the RNG sequence used for subsequent training.
        with torch.random.fork_rng(devices=[]):
            model.eval()
            x = torch.zeros(2, 20, 32)
            with torch.no_grad():
                y0 = model(x, torch.zeros(2, 20, 6))
                y1 = model(x, torch.ones(2, 20, 6))
                yn = model(x, None)
            if not (torch.isfinite(y0).all() and torch.equal(y0, y1) and torch.equal(y0, yn)):
                raise AssertionError('Technical-only stream isolation failed')
        model.train()
        report['technical_only_isolation_check'] = 'PASS'
        save()
        training_start=time.perf_counter()
        with isolated(out):
            result=train_pytorch_model(model,Progress(loaders['train'],'train'),Progress(loaders['val'],'validation'),
                test_loader=None,model_name=f'CORRECTED_I_seed{a.seed}',epochs=30,device=a.device,
        ...
        torch.save(dict(state_dict=result['model'].state_dict(),config=config,seed=a.seed,threshold=threshold,
                        technical_columns=adapter.TECH,fundamental_columns=[],
    ...
    print('PASS | corrected full I training, evaluation and indexed export',flush=True)
